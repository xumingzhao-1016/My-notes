...
| 函数类型     | WHERE 能用吗   | HAVING 能用吗 | 举例说明                                               |
| -------- | ----------- | ---------- | -------------------------------------------------- |
| 标量函数（行级） | ✅ 可以        | ✅ 可以       | `TIMESTAMPDIFF()`, `LENGTH()`, `DATE()`            |
| 聚合函数（组级） | ❌ 不可以       | ✅ 可以       | `SUM()`, `AVG()`, `COUNT()`                        |
| 逻辑表达式    | ✅ 可以        | ✅ 可以       | `=`, `>`, `LIKE`, `IN`, `BETWEEN`                  |
| 别名字段     | ✅ 可以（小心作用域） | ✅ 可以       | `HAVING avg_score > 80` 中 `avg_score` 来源于 `SELECT` |
